"""Simple experiments with linear or logistic regression.
...
# TODO(thunderhoser): Modularize and make titles nicer.
...
NUM_ITERATIONS_HELP_STRING = (
    'Number of iterations for linear regression.  This is the number of times '
    'that the weights will be updated.')
LEARNING_RATE_HELP_STRING = (
    'Learning rate for linear regression.  This is alpha in the weight-update '
    'rule.')
...
DEFAULT_OUTPUT_DIR_NAME = (
    '/home/ralager/Downloads/cs5033_fall2018/linear_regression_test')
...
INPUT_ARG_PARSER.add_argument(
    '--' + NOISE_STDEV_ARG_NAME, type=float, required=False, default=0.2,
    help=NOISE_STDEV_HELP_STRING)

INPUT_ARG_PARSER.add_argument(
    '--' + NUM_ITERATIONS_ARG_NAME, type=int, required=False, default=1000,
    help=NUM_ITERATIONS_HELP_STRING)

INPUT_ARG_PARSER.add_argument(
    '--' + LEARNING_RATE_ARG_NAME, type=float, required=False, default=0.01,
    help=LEARNING_RATE_HELP_STRING)

INPUT_ARG_PARSER.add_argument(
    '--' + LOGISTIC_ARG_NAME, type=int, required=False, default=1,
...
def _generate_data(
    ...
    if do_logistic:
        y_values = numpy.exp(y_values) / (1 + numpy.exp(y_values))
...
def _train_model(
    ...
    for i in range(num_iterations):
        ...
        # The "error" is either mean squared error or cross-entropy.
        if do_logistic:
            predicted_y_values = (
                numpy.exp(predicted_y_values) /
                (1 + numpy.exp(predicted_y_values))
            )
            error = -numpy.mean(
                y_values * numpy.log2(predicted_y_values) +
                (1. - y_values) * numpy.log2(1. - predicted_y_values)
            )
        else:
            error = numpy.mean((predicted_y_values - y_values) ** 2)
...
def _plot_results(x_values, y_values, weights, do_logistic, title_string,
                  output_file_name):
    """Plots results of linear regression.

    N = number of examples

    :param x_values: length-N numpy array of x-values (predictor values).
    :param y_values: length-N numpy array of y-values (actual target values).
    :param weights: Array produced by `_train_linear_regression`.
    ...
    if do_logistic:
        ...
        y_values_in_line = (
            numpy.exp(y_values_in_line) / (1 + numpy.exp(y_values_in_line))
        )
    else:
...
def _run(true_slope, true_intercept, noise_standard_deviation, num_iterations,
    ...
    """Simple experiments with linear regression.
    ...
    if do_logistic:
        predicted_training_y_values = (
            numpy.exp(predicted_training_y_values) /
            (1 + numpy.exp(predicted_training_y_values))
        )
        training_error = -numpy.mean(
            training_y_values * numpy.log2(predicted_training_y_values) +
            (1. - training_y_values) *
            numpy.log2(1. - predicted_training_y_values)
        )
    else:
        training_error = numpy.mean(
            (training_y_values - predicted_training_y_values) ** 2)

    training_title_string = (
        'Iterations = {0:d} ... learning rate = {1:.4e} ... training error = '
        '{2:.4e}'
    ).format(num_iterations, learning_rate, training_error)
    training_figure_file_name = (
        '{0:s}/training_num-iterations={1:d}_learning-rate={2:f}_logistic={3:d}'
        '.jpg'
    ).format(output_dir_name, num_iterations, learning_rate, do_logistic)
    ...
    if do_logistic:
        predicted_testing_y_values = (
            numpy.exp(predicted_testing_y_values) /
            (1 + numpy.exp(predicted_testing_y_values))
        )
        testing_error = -numpy.mean(
            testing_y_values * numpy.log2(predicted_testing_y_values) +
            (1. - testing_y_values) *
            numpy.log2(1. - predicted_testing_y_values)
        )
    else:
        testing_error = numpy.mean(
            (testing_y_values - predicted_testing_y_values) ** 2)

    testing_title_string = (
        'Iterations = {0:d} ... learning rate = {1:.4e} ... testing error = '
        '{2:.4e}'
    ).format(num_iterations, learning_rate, testing_error)
    testing_figure_file_name = (
        '{0:s}/testing_num-iterations={1:d}_learning-rate={2:f}_logistic={3:d}'
        '.jpg'
    ).format(output_dir_name, num_iterations, learning_rate, do_logistic)
...
if __name__ == '__main__':
    ...
    _run(
        ...
        do_logistic=getattr(INPUT_ARG_OBJECT, LOGISTIC_ARG_NAME),
